...
const ModalChangePassword = (props: any) => {
  ...
  const onFinishStep0 = async (values: any) => {
    ...
    const res = await sendRequest<IBackendRes<any>>({
      url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/forgotPassword`,
      method: "POST",
      body: {
        email,
      },
  ...
  const onFinishStep1 = async (values: any) => {
    ...
    if (newPassword !== confirmNewPassword) {
      notification.error({
        ...
        description: "Mật khẩu mới và xác nhận mật khẩu mới không chính xác",
      });
      return;
    }
    const res = await sendRequest<IBackendRes<any>>({
      url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/changePassword`,
      method: "POST",
      body: {
        code,
        newPassword,
